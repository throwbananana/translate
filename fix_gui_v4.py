...
GUI_PATH = "book_translator_gui.pyw"
...
def remove_nested_open_admin_audit(text: str) -> str:
    ...
    if end == -1:
        raise RuntimeError("Could not locate the end of the nested open_admin_audit block.")

    return text[:start] + text[end:]

def ensure_class_method(text: str) -> str:
    if "\n    def open_admin_audit(self):\n" in text:
        return text
    ...
    if idx == -1:
        raise RuntimeError("Could not locate insertion anchor for open_admin_audit.")
    return text[:idx] + "\n" + NEW_METHOD + text[idx:]
...
def main() -> int:
    ...
    if not gui.exists():
        print(f"[ERROR] File not found: {gui}")
        return 1

    backup = gui.with_name(gui.name + ".bak_v4")
    shutil.copy2(gui, backup)

    text = gui.read_text(encoding="utf-8")
    original = text

    text = remove_nested_open_admin_audit(text)
    text = ensure_class_method(text)

    gui.write_text(text, encoding="utf-8", newline="\n")
    test_file.write_text(TEST_FILE, encoding="utf-8", newline="\n")
