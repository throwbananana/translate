...
- 新增 `tests/test_translation_run_config.py`、`tests/test_run_guard.py`、`tests/test_batch_task.py`、`tests/test_provider_base.py`、`tests/test_openai_compatible_provider.py`、`tests/test_openai_compatible_factory.py`、`tests/test_engine_bridge.py` 与 `tests/test_translation_engine_adapter_bridge.py`，覆盖运行配置、停止保护、批量任务状态、Provider timeout/factory 默认值、engine bridge 和 TranslationEngine 接入行为
