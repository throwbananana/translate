...
- 新增 `providers/` adapter 基础层，包含统一 request/response/error 类型与 OpenAI-compatible provider timeout 处理
- 新增 `docs/upgrade-plan.md`，记录稳定性、controller 下沉、Provider Adapter、批量任务与发布化路线
- 新增 `tests/test_translation_run_config.py`、`tests/test_run_guard.py`、`tests/test_batch_task.py`、`tests/test_provider_base.py` 与 `tests/test_openai_compatible_provider.py`，覆盖运行配置、停止保护、批量任务状态和 Provider timeout
